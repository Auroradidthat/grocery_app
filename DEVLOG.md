...
- Local branches `dev` and `release` still exist (both at `8634d4b`, behind `main`, never pushed). Unknown whether `release` is intentional; delete with `git branch -d dev release` if not.
...
### Current state
- `v0.1.0` tagged and pushed; `main` matches `origin/main`.
